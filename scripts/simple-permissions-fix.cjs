...
async function simplePermissionsFix() {
  try {
    ...
    const currentPermissions = await sequelize.query('SELECT COUNT(*) as total FROM CGBRITO.PERMISSIONS', { type: 'SELECT' });
    const currentRolePermissions = await sequelize.query('SELECT COUNT(*) as total FROM CGBRITO.ROLE_PERMISSIONS', { type: 'SELECT' });
    
    console.log(`📊 Permisos actuales: ${currentPermissions[0][0].TOTAL}`);
    console.log(`📊 Role-Permissions actuales: ${currentRolePermissions[0][0].TOTAL}`);
    
    // 2. Crear nuevos permisos (sin eliminar los existentes)
    ...
    for (const permission of standardPermissions) {
      try {
        ...
        if (existing[0].length === 0) {
          ...
          console.log(`  ✅ Creado: ${permission.name}`);
    ...
    console.log(`✅ ${newPermissionsCreated} nuevos permisos creados`);
    ...
    console.log(`  - Permisos disponibles: ${Object.keys(permissionMap).length}`);
    console.log(`  - Roles disponibles: ${Object.keys(roleMap).length}`);
    
    // 4. Asignar permisos a roles (sin eliminar asignaciones existentes)
      ...
      for (const permissionName of permissionNames) {
        ...
        try {
          ...
          const existingAssignment = await sequelize.query(
            'SELECT ID FROM CGBRITO.ROLE_PERMISSIONS WHERE ROLE_ID = :roleId AND PERMISSION_ID = :permissionId',
            {
          ...
          if (existingAssignment[0].length === 0) {
            ...
            console.log(`  ⚠️ Ya asignado: ${permissionName}`);
            skippedAssignments++;
          }
        } catch (error) {
    ...
    console.log(`\n✅ Total de nuevas asignaciones: ${totalAssignments}`);
    console.log(`⚠️ Asignaciones ya existentes: ${skippedAssignments}`);
