...
async function fixPermissionsConstraints() {
  try {
    ...
    console.log('\n🔍 Verificando restricciones existentes...');
    const constraintsResult = await sequelize.query(`
      ...
      AND TABLE_NAME IN ('ROLE_PERMISSIONS', 'ROLE_MENU_PERMISSIONS')
      ORDER BY TABLE_NAME
    `, { type: 'SELECT' });
    
    console.log('Restricciones encontradas:');
    constraintsResult[0].forEach(constraint => {
      console.log(`  - ${constraint.TABLE_NAME}: ${constraint.CONSTRAINT_NAME} -> ${constraint.R_CONSTRAINT_NAME}`);
    });
    
    // 2. Deshabilitar restricciones temporalmente
    console.log('\n🔓 Deshabilitando restricciones temporalmente...');
    try {
      await sequelize.query('ALTER TABLE CGBRITO.ROLE_PERMISSIONS DISABLE CONSTRAINT FK_RP_ROLE');
      console.log('✅ Restricción FK_RP_ROLE deshabilitada');
    } catch (e) {
      console.log('⚠️ No se pudo deshabilitar FK_RP_ROLE:', e.message);
    }
    
    try {
      await sequelize.query('ALTER TABLE CGBRITO.ROLE_PERMISSIONS DISABLE CONSTRAINT FK_RP_PERMISSION');
      console.log('✅ Restricción FK_RP_PERMISSION deshabilitada');
    } catch (e) {
      console.log('⚠️ No se pudo deshabilitar FK_RP_PERMISSION:', e.message);
    }
    
    // 3. Limpiar datos existentes
    console.log('\n🧹 Limpiando datos existentes...');
    await sequelize.query('DELETE FROM CGBRITO.ROLE_PERMISSIONS');
    console.log('✅ ROLE_PERMISSIONS limpiado');
    
    await sequelize.query('DELETE FROM CGBRITO.PERMISSIONS');
    console.log('✅ PERMISSIONS limpiado');
    
    // 4. Crear nuevos permisos estándar
    ...
    // 5. Obtener IDs de permisos y roles
    ...
    // 7. Rehabilitar restricciones
    console.log('\n🔒 Rehabilitando restricciones...');
    try {
      await sequelize.query('ALTER TABLE CGBRITO.ROLE_PERMISSIONS ENABLE CONSTRAINT FK_RP_ROLE');
      console.log('✅ Restricción FK_RP_ROLE rehabilitada');
    } catch (e) {
      console.log('⚠️ No se pudo rehabilitar FK_RP_ROLE:', e.message);
    }
    
    try {
      await sequelize.query('ALTER TABLE CGBRITO.ROLE_PERMISSIONS ENABLE CONSTRAINT FK_RP_PERMISSION');
      console.log('✅ Restricción FK_RP_PERMISSION rehabilitada');
    } catch (e) {
      console.log('⚠️ No se pudo rehabilitar FK_RP_PERMISSION:', e.message);
    }
